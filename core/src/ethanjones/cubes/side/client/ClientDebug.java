...
public class ClientDebug {
  ...
  public static void update() {
    ...
    debugString = Branding.DEBUG + lineSeparator + str + "FPS:" + fps.current + " AFPS:" + fps.average + " MS:" + String.format("%01d", loop.current) + " AMS:" + String.format("%01d", loop.average) + lineSeparator + "RAM T:" + Memory.max + "MB F:" + Memory.totalFree + "MB U:" + Memory.used + "MB" + lineSeparator + lineSeparator + "P X:" + String.format("%.2f", p.x) + " Y:" + String.format("%.2f", p.y) + " Z:" + String.format("%.2f", p.z) + lineSeparator + "A X:" + CoordinateConverter.area(p.x) + " Y:" + CoordinateConverter.area(p.y) + " Z:" + CoordinateConverter.area(p.z) + lineSeparator + "Z X:" + CoordinateConverter.zone(CoordinateConverter.area(p.x)) + " Z:" + CoordinateConverter.zone(CoordinateConverter.area(p.z)) + lineSeparator + "D X:" + String.format("%02f", Cubes.getClient().player.angle.x) + " Y:" + String.format("%02f", Cubes.getClient().player.angle.y) + " Z:" + String.format("%02f", Cubes.getClient().player.angle.z) + lineSeparator + lineSeparator + "C:" + GLProfiler.calls + lineSeparator + "DC:" + GLProfiler.drawCalls + lineSeparator + "S:" + GLProfiler.shaderSwitches + lineSeparator + "T:" + GLProfiler.textureBindings + lineSeparator + "V:" + GLProfiler.vertexCount.latest + lineSeparator;
